...
exports.hyperlinks = function () {
	...
	for (var i = 0; i < videos.length; i++) {
		if (videos[i].assessment !== null) {
			...
			for (var j = 0; j < videos[i].assessment.length; j++) {
				console.log(videos[i].assessment[j].author)
			}
		}
	}
	return
	for (var i = 0; i < videos.length; i++) {
		if (videos[i].hyperlinks !== null) {
			...
			for (var j = 0; j < videos[i].hyperlinks.length; j++) {
				console.log(videos[i].hyperlinks[j].target)
			}
		}
	}
	...
	for (var i = 0; i < videos.length; i++) {
		if (videos[i].comments !== null) {
			for (var j = 0; j < videos[i].comments.length; j++) {
				console.log(videos[i].comments[j].author)
			}
		}
	}
}
...
exports.assessment = function () {
	...
	for (var i = 0; i < videos.length; i++) {
		sum = sum + videos[i].assessment.length;
		console.log(videos[i].id, videos[i].assessment.length)
		for (var j = 0; j < videos[i].assessment.length; j++) {
			...
			for (var answ = 0; answ < videos[i].assessment[j].title.answ.length; answ++) {
				...
				} else { // fill-in questions
					out += lfn + ',,,,,,--,"' + videos[i].assessment[j].title.answ[answ].answ + '"\n';
				}
				//console.log(correct)
				//console.log(moment(videos[i].assessment[j].date,'x').format())
			}
			lfn++;
		}
	}
...
/*
**/
exports.media_affinity = function () {
	var
		tests = JSON.parse(fs.readFileSync('./input/etuscript2015/mongodb/tests.json')),
		res = {}
		;
	for (var i = 0; i < tests.length; i++) {
		if (tests[i].type === "prepost" && tests[i].user !== 'SEIDEL') {
			res[tests[i].user] = [];
			for (var j = 0; j < tests[i].user_results.length; j++) {

				// iterate tasks
				for (var t = 0; t < 12; t++) {
					if (tests[i].user_results[j]['Task-' + t] !== undefined) {
						for (var k = 0; k < tests[i].user_results[j]['Task-' + t].length; k++) {
							if (tests[i].user_results[j]['Task-' + t][k].value !== undefined) {
								res[tests[i].user][t] = parseInt(tests[i].user_results[j]['Task-' + t][k].option.replace(t + 'answ', ''))
							}
						}
					}
				}// end task	
...
convertLogFromJSON = function (conf) {
	...
	LogExt.remove({}, function (err) {
		...
		fileStream.pipe(JSONStream.parse('*')).pipe(es.through(function (data) {
			...
			o.user = data.user;
			o.user_name = u.name == undefined ? 'null' : u.name;
			o.user_gender = u.gender;
			o.user_culture = u.culture;

			// extent group data
			console.log(o.phase + '___' + u.groups + '__' + data.user)
			o.group_name = u.groups !== undefined ? u.groups[(o.phase - 1)] : 'null'
			...
			if (typeof data.action.values === 'object' && data.action.values.length === 1) {
				...
				if (data.action.action === 'skip-back') {
					...
					console.log('missed_1 action context/type ', o.action_context, o.action_type)
				}
				...
				console.log('missed_2 action context/type ', data.action.context, data.action.action)
			}
			...
			// 
			if (projectConfiguration.exclude_groups.indexOf(o.group) === -1 && o.group !== "undefined" && o.group_name !== "null") {
				...
				o.user_name = o.user_name;//crypto.createHash('md5').update( o.user_name ).digest('hex');//user_name: 'Andre',
				o.playback_time = o.playback_time === 0 ? 0 : o.playback_time.toFixed(3);//playback_time: 131.101927,
				o.action_value = validate(o.action_value);//action_value: '26',
				o.action_artefact = validate(o.action_artefact);//o.action_artefact = data.action.values[0];
				o.action_artefact_id = validate(o.action_artefact_id);//o.action_artefact_id = data.action.values[0].hashCode();
				o.action_artefact_author = validate(o.action_artefact_author);//o.action_artefact_author = data.action.values[1];
				o.action_artefact_time = validate(o.action_artefact_time);//o.action_artefact_time = parseInt(data.action.values[2]); 
				o.action_artefact_response = validate(o.action_artefact_response);//o.action_artefact_response = data.action.values[3];

				o.ua_browser = validate(o.ua_browser);//ua_browser: 'Firefox',
				o.ua_browser_engine = validate(o.ua_browser_engine);//ua_browser_engine: 'Gecko_41.0',
				o.ua_browser_version = validate(o.ua_browser_version);//ua_browser_version: '41',
				o.ua_device = validate(o.ua_device);//ua_device: -88,
				o.ua_os = validate(o.ua_os);//ua_os: 'Windows',
				o.ua_os_version = validate(o.ua_os_version);//ua_os_version: 'Vista',
...
function saveLogEntry(file, es) {
	// save it
	new LogExt(file).save(function (err, entry, count) {
		if (err) {
			console.log(err)
		} else {
			es.resume();
